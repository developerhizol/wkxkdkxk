...
def tint_image(img: Image.Image, hex_color: Optional[str]) -> Image.Image:
...
def tint_lottie(lottie_json: dict, hex_color: Optional[str]) -> dict:
    if hex_color is None:
        return lottie_json
    r, g, b = hex_to_rgb(hex_color)
...
def _replace_group_text(grp: dict, bounds: tuple, new_text: str, font_path: str) -> bool:
    ...
    new_shapes = _text_to_lottie_shapes(new_text, font_path, cx, cy, height, max_width=width * 1.2)
    if not new_shapes:
        return False
...
def _replace_username(lottie: dict, new_text: str, font_path: str) -> bool:
    ...
    for bounds, grp in _find_named_groups(lottie, ("USERNAME",)):
        ...
        width = max(abs(x2 - x1), 1.0)
        new_shapes = _text_to_lottie_shapes(new_text, font_path, cx, cy, height, max_width=width * 1.2)
        if new_shapes and _replace_group_shapes(grp, new_shapes):
            changed = True
    return changed
...
def _replace_passport_username(
    ...
    for bounds, grp in username_groups:
        ...
        height = max(abs(y2 - y1), 1.0)
        width = max(abs(x2 - x1), 1.0)
        username_shapes = _text_to_lottie_shapes(new_text, font_path, cx, cy, height, max_width=width * 1.2)
...
def _inject_passport_username_overlay(
    ...
    x1, y1, x2, y2 = username_bounds
    cx = (x1 + x2) / 2.0
    cy = (y1 + y2) / 2.0
    height = max(abs(y2 - y1), 1.0)
    width = max(abs(x2 - x1), 1.0)
    text_shapes = _text_to_lottie_shapes(new_text, font_path, cx, cy, height, max_width=width * 1.2)
    if not text_shapes:
        return False
...
def _text_shapes_in_bounds(
    ...
    width = max(x2 - x1, 5.0)
    return _text_to_lottie_shapes(text, font_path, cx, cy, height, max_width=width * 1.2)
...
# ─────────────────────────────────────────────────────────────────────
# ПАТЧ: transform path shapes (X / Y / Scale / Rotation)
# ─────────────────────────────────────────────────────────────────────

def _transform_lottie_paths(
    shapes: List[dict],
    cx: float,
    cy: float,
    dx: float,
    ...
    """
    Применяет affine-трансформацию (сдвиг / масштаб / поворот) к path-shape'ам
    относительно точки (cx, cy). Возвращает новые shape'ы, не мутируя исходные.
    """
    if (
        abs(dx) < 1e-9
        and abs(dy) < 1e-9
    ...
    def vec(vx: float, vy: float) -> List[float]:
    ...
    for shape in shapes:
        ...
        new_shape["ks"] = {
            ...
            "k": {
                "c": k.get("c", True),
                "v": [pt(v[0], v[1]) for v in k.get("v", [])],
                "i": [vec(i[0], i[1]) for i in k.get("i", [])],
                "o": [vec(o[0], o[1]) for o in k.get("o", [])],
            },
        }
...
# ─────────────────────────────────────────────────────────────────────
# ПАТЧ: customize_tgs_template с offset / scale / rotation и font_path_override
# ─────────────────────────────────────────────────────────────────────
...
def customize_tgs_template(
    ...
    font_path = font_path_override or get_font_path(font_id)
    if font_path is None:
    ...
    new_shapes = _compose_brand_shapes(
        text, font_path, bounds, logo_svg, canvas_size=canvas_size
    )
    if not new_shapes:
        raise ValueError("Не удалось подготовить текст или логотип для шаблона.")

    if offset_x or offset_y or abs(scale - 1.0) > 1e-9 or rotation:
        tx_cx = (bounds[0] + bounds[2]) / 2.0
        tx_cy = (bounds[1] + bounds[3]) / 2.0
        new_shapes = _transform_lottie_paths(
            new_shapes, tx_cx, tx_cy, offset_x, offset_y, scale, rotation
        )

    if skip_color or hex_color is None:
        tinted = lottie
    else:
    ...
    if enforce_size_limit and len(out_data) > MAX_ANIMATED_STICKER_BYTES:
        raise ValueError(
            ...
            f"{len(out_data)} bytes > {MAX_ANIMATED_STICKER_BYTES}"
        )
    return out_data
...
def build_tgs_preview_tile(
    ...
    actual_hex_color = None if skip_color else hex_color

    source = render_tgs_preview_image(
        tgs_bytes=tgs_bytes,
        text=text,
        hex_color=actual_hex_color,
        font_id=font_id,

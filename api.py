...
def _build_pack_name(slug: str) -> str:
    slug = "".join(c for c in slug.lower() if c.isalnum() or c == "_")
    if not slug or not slug[0].isalpha():
        slug = "pack" + slug
    max_base = 64 - len(PACK_NAME_SUFFIX)
    return (slug[:max_base] + PACK_NAME_SUFFIX)
...
async def api_generate(
    ...
    try:
        ...
        pack_name = _build_pack_name(slug)
        ...
        for it, r in zip(items, rendered):
            ...
            if len(r) > MAX_ANIMATED_STICKER_BYTES:
                log.error(
                    "item too big: %d > %d",
                    len(r), MAX_ANIMATED_STICKER_BYTES,
                )
                continue
        ...
        for entry in ok_items[1:]:
            ...
            await tg.add_sticker_to_set(
                uid, pack_name, entry["bytes"], emoji
            )
